...
let rec freevars (e: expr) : string list =
  ...
  | Not e1 -> freevars e1
  | Let (s, e1, e2) -> List.filter (fun v -> v <> s) (freevars e1 @ freevars e2)
  | Lam (s1, e1) ->
    List.filter (fun s -> s <> s1) (freevars e1)
  | App (e1, e2) ->
    freevars e1 @ freevars e2
  | If (e1, e2, e3) -> freevars e1 @ freevars e2 @ freevars e3
  | LetRec (s, e1, e2) -> List.filter (fun v -> v <> s) (freevars e1 @ freevars e2)
...
let rec eval (e:expr) (env: environment) : value =
  ...
  | Div (e1, e2) ->
     ( match eval e1 env, eval e2 env with
       | Int v1, Int v2 -> 
        if v2 = 0 then raise (DivisionByZero (Int v1))
        else Int (v1 / v2)
  ...
  | If (e1, e2, e3) ->
    if eval e1 env = Bool true then eval e2 env else eval e3 env
  | LetRec (s, e1, e2) ->
    (match eval e1 env with
    | Closure (arg, body, env1) ->
        let recursive_env = (s, Closure (arg, body, env1)) :: env in
        let new_env = (arg, Closure (arg, body, env1)) :: recursive_env in
        eval e2 new_env
    | _ -> raise (IncorrectType "LetRec"))
